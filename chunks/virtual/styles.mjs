const interopDefault = r => r.default || r || [];
const styles = {
  "pages/fetch.vue": () => import('../build/fetch-styles.B_fuTj3x.mjs').then(interopDefault),
  "pages/[id].vue": () => import('../build/_id_-styles.D2DBsnji.mjs').then(interopDefault),
  "pages/index.vue": () => import('../build/index-styles.MuChuUMF.mjs').then(interopDefault),
  "../node_modules/nuxt/dist/app/components/error-404.vue": () => import('../build/error-404-styles.CbLQIMfl.mjs').then(interopDefault),
  "../node_modules/nuxt/dist/app/components/error-500.vue": () => import('../build/error-500-styles.CB5z51D_.mjs').then(interopDefault)
};

export { styles as default };
//# sourceMappingURL=styles.mjs.map
